...
import br.com.renan.twgerenciadortarefas.repositorios.RepositorioTarefa;
import br.com.renan.twgerenciadortarefas.servicos.ServicoUsuario;

@Controller
@RequestMapping("/tarefas")
public class TarefaController {

	// Injenção de dependencia/ ingeção via spring
	@Autowired
	...
	public ModelAndView listar(HttpServletRequest request) {
		...
		String emialUsuario = request.getUserPrincipal().getName();
		mv.addObject("tarefas", repositorioTarefa.carregarTarefasPorUsuario(emialUsuario));
		return mv;
	}
	...
	public ModelAndView inserir(@Valid Tarefa tarefa, BindingResult result, HttpServletRequest request) {
		...
		if (tarefa.getDataExpiracao() == null) {
			result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data de Expiração é obrigatoriaol");

		} else {
			if (tarefa.getDataExpiracao().before(new Date())) {
				result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida",
						"A data de expiração não pode ser anterior a atual");
		...
		if (result.hasErrors()) {
			...
			String emialUsuario = request.getUserPrincipal().getName();
			Usuario usuarioLogado = servicoUsuario.procurarPorEmail(emialUsuario);
